...
export const CANCELLABLE_STATUSES = ["Placed", "Accepted", "Processing"] as const;
...
export type Order = { id: string; orderId: string; invoiceNumber: string; customer: OrderCustomer; address: Address; items: CartItem[]; totals: OrderTotals; paymentMethod: PaymentMethod; paymentStatus: "Pending" | "Paid" | "Failed" | "Refunded"; transactionId?: string; providerOrderId?: string; status: OrderStatus; createdAt?: { seconds: number }; updatedAt?: { seconds: number }; cancelledAt?: { seconds: number }; estimatedDelivery: string };
